...
layui.use(['table', 'carousel', 'laydate', 'form', 'layer'], function () {
    ...
    form.on('radio(lockDemo)', function (obj) {
        if (radiovalue === obj.value) {
            ...
            purchaseId = this.value;
            purchaseName = $(this).parent().parent().parent().find("td[data-field='orgName'] div").text();
        }
        ...
        success: function (data) {
            ...
            if (data.status == 200) {
                ...
                $.each(data.data, function (index, item) {
                    ...
                    if (status == 0) {
                        $("#unconfirmed").html(count+"笔共"+tool+"元");
                    } else if (status == 1) {
                        $("#confirmed").html(count+"笔共"+tool+"元");
                    } else {
                        $("#rejected").html(count+"笔共"+tool+"元");
                    }

                    totol += tool;
                    num += count;
                });
                $("#all").html(num+"笔共"+totol+"元");
            } else {
        ...
        success: function (data) {
            if (data.status == 200) {
                $.each(data.data, function (index, item) {
                    ...
                    var count = item.count;
                    if (status == 0) {
                        $("#uncleared").html(count+"笔共"+tool+"元");
                    } else if (status == 1) {
                        $("#launched").html(count+"笔共"+tool+"元");
                    } else {
                        $("#cleared").html(count+"笔共"+tool+"元");
                    }
                });
            } else {
                layer.alert(data.msg);
...
$(function () {
    ...
    $('.step1').click(function () {
        $(".purchaseName").html(purchaseName);
        swiper.slideNext()
    })
